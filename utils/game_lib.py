...
def get_children_nodes(parent_node, i, j, k, z, totalNumberOfVisited):
    children_nodes = []
...
# We want to minimize our heuristic. For each candy on the first or last row (the ones that need to match symetrically), we check if any of the neighbouring candies
# are a match
def get_heuristic_value(board, i, j, k):
    heuristic = 100
    for x in range(5):
        if x in range(1,4):
            if board[0][x-i] == board[2][x] or board[0][x] == board[2][x] or board[0][x+i] == board[2][x]:
                heuristic -= k
            elif board[1][x-i] == board[2][x] or board[1][x] == board[2][x] or board[1][x+i] == board[2][x]:
                heuristic -= k
            elif board[2][x - 1] == board[2][x] or board[2][x + 1] == board[2][x]:
                heuristic -= k

            elif board[2][x-i] == board[2][x] or board[2][x] == board[0][x] or board[2][x+i] == board[0][x]:
                heuristic -= k
            elif board[1][x-i] == board[0][x] or board[1][x] == board[0][x] or board[1][x+i] == board[0][x]:
                heuristic -= k
            elif board[0][x - 1] == board[0][x] or board[0][x + 1] == board[0][x]:
                heuristic -= k
        elif x == 0:
            if board[0][x] == board[2][x] or board[0][x+i] == board[2][x]:
                heuristic -= k
            elif board[1][x] == board[2][x] or board[1][x + 1] == board[2][x]:
                heuristic -= k
            elif board[2][x+i] == board[2][x]:
                heuristic -= k

            if board[2][x] == board[0][x] or board[2][x+i] == board[0][x]:
                heuristic -= k
            elif board[1][x] == board[0][x] or board[1][x + 1] == board[0][x]:
                heuristic -= k
            elif board[0][x+i] == board[0][x]:
                heuristic -= k
        elif x == 4:
            if board[0][x] == board[2][x] or board[0][x-i] == board[2][x]:
                heuristic -= k
